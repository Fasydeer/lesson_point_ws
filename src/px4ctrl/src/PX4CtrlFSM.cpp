...
void PX4CtrlFSM::process()
{
	...
	// 	debug_msg = controller.calculateControl(des, odom_data, u);
	...
	{
		publish_position_ctrl(des, now_time);
	}
...
{
	Eigen::Vector3d idling(0,0,0.05);
	// Desired_State_t des;
	des.p = takeoff_land.start_pose.head<3>()+idling;
}
...
void PX4CtrlFSM::land_detector(const State_t state, const Desired_State_t &des, const Odom_Data_t &odom)
{
	...
	void PX4CtrlFSM::manual_flag_cb(const std_msgs::UInt8 msg)
{
	geometry_msgs::PoseStamped now_pose;
    if(msg.data)
    {
        now_pose.pose.position.x=odom_data.p[0];
		now_pose.pose.position.y=odom_data.p[1];
		now_pose.pose.position.z=odom_data.p[2];
...
{
	Desired_State_t des;
	des.p.x() = hover_pose[0];
	des.p.y() = hover_pose[1];
	des.p.z() = hover_pose[2];
...
{
	Desired_State_t des;
	des.p(0) = cmd_data.msg.pose.position.x;
	des.p(1) = cmd_data.msg.pose.position.y;
	des.p(2) = cmd_data.msg.pose.position.z;
	//std::cout << "cmd_des=" << cmd_data.msg.pose.position.y << std::endl;
	// des.v = cmd_data.v;
	// des.a = cmd_data.a;
	// des.j = cmd_data.j;
...
// 	case 1:
// 	//碰撞前侧
// 			des.p[0]=odom_data.p[0]-3e-1;
// 			des.p[1]=cmd_data.p[1];
// 			//des.p[2]=cmd_data.p[2];
// 			des.p[2]=1.5;
// 			// des.v = Eigen::Vector3d::Zero();
// 			// des.a = Eigen::Vector3d::Zero();
// 			// des.j = Eigen::Vector3d::Zero();
...
// 	case 3:
// 	//碰撞左侧
// 			des.p[0]=cmd_data.p[0];
// 			des.p[1]=cmd_data.p[1]-3e-1;
// 			//des.p[2]=cmd_data.p[2];
// 			des.p[2]=1.5;
// 			// des.v = Eigen::Vector3d::Zero();
// 			// des.a = Eigen::Vector3d::Zero();
...
// 	default:
// 		ROS_INFO("collision_detection_error");
// 		break;
// 	}
...
int PX4CtrlFSM::collision_edge()
{
	if(odom_data.p[0]>=6.5)
	{
		ROS_INFO("collision front");
		return 1;
	}
	else if(odom_data.p[0]<=-6.5)
	{
		ROS_INFO("collision rear");
		return 2;
	}
	else if(odom_data.p[1]>=3.5)
	{
		ROS_INFO("collision left");
		return 3;
	}
	else if(odom_data.p[1]<=-3.5)
	{
		ROS_INFO("collision right");
		return 4;
	}
	else
	{
...
Desired_State_t PX4CtrlFSM::get_rotor_speed_up_des(const ros::Time now)
{
	...
	Desired_State_t des;
	Eigen::Vector3d speed_up(0,0,0.2);
	des.p = takeoff_land.start_pose.head<3>()+speed_up;
	// des.v = Eigen::Vector3d::Zero();
...
void PX4CtrlFSM::set_hov_with_odom()
{
	...
	hover_pose(0)=odom_data.p(0);
	hover_pose(1)=odom_data.p(1);
...
// }
 ...
 void PX4CtrlFSM::publish_position_ctrl(const Desired_State_t &des, const ros::Time &stamp)
 {
	...
	msg.position.x=des.p[0];
	msg.position.y=des.p[1];
	msg.position.z=des.p[2];
	msg.yaw=des.yaw;
	//msg.yaw=des.fromQuaternion2yaw(odom_data.q);

	//  msg.pose.position.x=des.p[0];
	//  msg.pose.position.y=des.p[1];
	//  msg.pose.position.z=des.p[2];
	//  msg.pose.orientation.w=odom_data.q.w();
	//  msg.pose.orientation.x=odom_data.q.x();
	//  msg.pose.orientation.y=odom_data.q.y();
	//  msg.pose.orientation.z=odom_data.q.z();
	ctrl_FCU_pub.publish(msg);
 }

void PX4CtrlFSM::publish_trigger(const geometry_msgs::PoseStamped &odom_msg)
{
	geometry_msgs::PoseStamped msg;
...
bool PX4CtrlFSM::toggle_offboard_mode(bool on_off)
{
	...
	if (on_off)
	{
		...
		if (state_data.state_before_offboard.mode == "OFFBOARD") // Not allowed
			state_data.state_before_offboard.mode = "ALTCTL";
