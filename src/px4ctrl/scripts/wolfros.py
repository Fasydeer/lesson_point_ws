...
filename = '/home/uav/twf_ws/src/test_wifi/scripts/caiji.csv'      
M = np.loadtxt(filename,delimiter=',')
# signal_st = uav_sgn_status()
signal_st1= uav_sgn_status()
# signal_st2=uav_sgn_status()
signal_st3=uav_sgn_status()
signal_st4=uav_sgn_status()
signal_st5=uav_sgn_status()
...
def point():
    global Point,E,X,Y,signal_st1,signal_st2,signal_st3,signal_st4,signal_st5
   #  global Point,E,X,Y,signal_st1,signal_st2,signal_st4,signal_st5,signal_st3
    rospy.init_node('wolf_pos', anonymous=True)
    pub1 = rospy.Publisher('/uav1/target_point', PoseStamped, queue_size=10) 
   #  pub2 = rospy.Publisher('/uav2/target_point', PoseStamped, queue_size=10) 
    pub3 = rospy.Publisher('/uav3/target_point', PoseStamped, queue_size=10) 
   ...
   #          break
    while (True):
      # if(signal_st1.rssi!=0 and signal_st4.rssi!=0 and signal_st3.rssi!=0 ):
      #    break
      if(signal_st3.rssi!=0 ):
         break

    [pose1.pose.position.x,pose1.pose.position.y,pose1.pose.position.z] = [round(M[0][0],2),round(M[0][1],2),1]
   #  [pose2.pose.position.x,pose2.pose.position.y,pose2.pose.position.z] = [round(M[0][2],2),round(M[0][3],2),1]
    #[pose3.pose.position.x,pose3.pose.position.y,pose3.pose.position.z] = [round(M[0][2],2),round(M[0][3],2),1]
    #[pose3.pose.position.x,pose3.pose.position.y,pose3.pose.position.z] = [round(M[0][0],2),round(M[0][1],2),1]
    #[pose4.pose.position.x,pose4.pose.position.y,pose4.pose.position.z] = [round(M[0][4],2),round(M[0][5],2),1]
    #[pose5.pose.position.x,pose5.pose.position.y,pose5.pose.position.z] = [round(M[0][6],2),round(M[0][7],2),1]

    #E = [signal_st1.rssi,signal_st2.rssi,signal_st3.rssi,signal_st4.rssi,signal_st5.rssi]
   #  E = [signal_st1.rssi,signal_st3.rssi,signal_st4.rssi]
   ...
   #  Y = [round(M[0][1],2),round(M[0][3],2),round(M[0][5],2)]
    ...
    while not rospy.is_shutdown():
         ...
         for i in range(1,M.shape[0]):
              # [pose1.pose.position.x,pose1.pose.position.y,pose1.pose.position.z] = [round(M[i][0],2),round(M[i][1],2),1]
               # [pose2.pose.position.x,pose2.pose.position.y,pose2.pose.position.z] = [round(M[i][2],2),round(M[i][3],2),1]
               #[pose3.pose.position.x,pose3.pose.position.y,pose3.pose.position.z] = [round(M[i][2],2),round(M[i][3],2),1]
               [pose3.pose.position.x,pose3.pose.position.y,pose3.pose.position.z] = [round(M[i][0],2),round(M[i][1],2),1]
              # [pose4.pose.position.x,pose4.pose.position.y,pose4.pose.position.z] = [round(M[i][4],2),round(M[i][5],2),1]
               #[pose5.pose.position.x,pose5.pose.position.y,pose5.pose.position.z] = [round(M[i][6],2),round(M[i][7],2),1]
               print(i,pose3.pose.position,signal_st3.rssi)
               pub1.publish(pose1)
               # pub2.publish(pose2)
               pub3.publish(pose3)
               # print(pose3)
               ...
               rate.sleep()
               time.sleep(0.8)
               #temp_E = [signal_st1.rssi,signal_st3.rssi,signal_st4.rssi]
               temp_E = [signal_st3.rssi]
               E = E+temp_E
               #temp_X = [round(M[i][2],2),round(M[i][4],2),round(M[i][6],2),round(M[i][8],2)]
    ...
    np.savetxt('/home/uav/twf_ws/src/test_wifi/scripts/xcsv',X,delimiter=',')
    np.savetxt('/home/uav/twf_ws/src/test_wifi/scripts/y.csv',Y,delimiter=',')
    np.savetxt('/home/uav/twf_ws/src/test_wifi/scripts/E.csv',E,delimiter=',')
    print(len(X),len(Y),len(E))



    location = np.hstack((X, Y))
    x_new, y_new = np.meshgrid(np.linspace(-5,5,60), np.linspace(-3,3,60))

    data = interpolate.griddata(location, E, (x_new, y_new), method='linear')

    e_new = data[:, :, 0]

   # np.savetxt('/home/uav/twf_ws/src/test_wifi/scripts/M3.csv',e_new,delimiter=',')

    im =  plt.imshow(e_new,origin='lower',extent=[-2.8,2.8,-2.8,2.8],cmap=mpl.cm.jet)
    plt.colorbar(im)

    plt.figure()
    plt.scatter(X, Y, s=20,  c=E, cmap=mpl.cm.jet)
    plt.show()
